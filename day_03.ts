...
export const part1 = (input: string) => {
  ...
  lines.forEach((line: string) => {
    ...
    const commonChars = new Set(firstHalf.filter((char) => secondHalf.includes(char)));
...
export const part2 = (input: string) => {
  ...
  blocks.forEach((block: string[]) => {
    const commonChars = new Set(block[0].split('').filter((char) => block[1].includes(char) && block[2].includes(char)));
    commonChars.forEach((char) => {
      value += charToNumber(char);
    });
  });
